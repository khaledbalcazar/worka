-- ============================================================
-- Worka — Migración 024: tiempo por etapa y precios de Evaluar
--
-- Un test de razonamiento sin límite de tiempo mide bastante menos de lo que
-- dice medir: cualquiera puede pensar diez minutos una serie o buscar la
-- respuesta. Esta migración agrega el cronómetro y, sobre todo, guarda cuánto
-- tardó cada persona en cada etapa: sin ese dato no hay forma de distinguir a
-- quien resolvió de quien tocó cualquier cosa en ocho segundos.
--
...
-- Segundos que tardó en cada etapa: { "<stage_id>": 247 }
alter table evaluar_participants
  add column if not exists stage_times jsonb not null default '{}'::jsonb;

-- Precios de los planes, editables desde /admin como cualquier otro ajuste.
insert into site_settings (key, value)
values
  ('evaluar_precio_esencial', '390000'),
  ('evaluar_precio_profesional', '790000'),
  ('evaluar_precio_corporativo', '')
on conflict (key) do nothing;

-- ============================================================
-- La corrección ahora también registra el tiempo.
--
-- El parámetro nuevo va con DEFAULT para no romper a quien ya esté llamando
...
begin
  ...
  v_times := coalesce(v_participant.stage_times, '{}'::jsonb);

  if p_seconds is not null then
    v_times := jsonb_set(
      v_times, array[p_stage_id::text], to_jsonb(greatest(p_seconds, 0))
    );
  end if;
  ...
  loop
    ...
    if v_q.kind = 'likert' then
      ...
      v_points := coalesce(
        (v_q.option_scores ->> (v_value #>> '{}'))::numeric, 0
      );
    ...
    values (
      v_participant.id, v_q.id, coalesce(v_value, 'null'::jsonb),
      coalesce(v_points, 0)
    )
...
-- evaluar_load debe entregar si la etapa es cronometrada.
...
grant execute on function evaluar_submit_stage(text, uuid, jsonb, int) to anon, authenticated;
